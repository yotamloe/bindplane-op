...
bad_pki() {
    # certificate authority
    openssl genrsa -out tls/bad-ca.key 4096
    openssl req -new -x509 -sha256 \
        -key tls/bad-ca.key \
        -out tls/bad-ca.crt \
        -days 10950 \
        -subj "/C=US/ST=Michigan/L=GrandRapids/O=observIQ/OU=bad/CN=bad-ca"

    # certificate
    openssl genrsa -out tls/bad.key 4096
    openssl req -new -key tls/bad.key -out tls/bad.csr \
        -subj "/C=US/ST=Michigan/L=GrandRapids/O=observIQ/OU=bad/CN=localhost" \
        -addext "subjectAltName=DNS:localhost,DNS:bindplane.bindplane-dev.svc.cluster.local"
    openssl ca \
        -create_serial \
        -cert tls/bad-ca.crt \
        -keyfile tls/bad-ca.key \
        -days 9125 \
        -in tls/bad.csr \
        -batch \
        -config tls/openssl.conf \
    ...
    # append ca.crt to server certificate
    cat tls/bad-ca.crt >> tls/bad.crt    

    # client cert
    openssl genrsa -out tls/bad-client.key 4096
    openssl req -new -key tls/bad-client.key -out tls/bad-client.csr \
        -subj "/C=US/ST=Michigan/L=GrandRapids/O=observIQ/OU=bad/CN=bad-dev-client"
    openssl ca \
        -create_serial \
        -cert tls/bad-ca.crt \
        -keyfile tls/bad-ca.key \
        -days 9125 \
        -in tls/bad-client.csr \
        -batch \
        -config tls/openssl.conf \
        -out tls/bad-client.crt
}
